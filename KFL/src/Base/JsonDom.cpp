...
namespace
{
	...
	std::unique_ptr<JsonValue> CreateJsonValueFromRapidJsonValue(JsonDocument& doc, rapidjson::Value& value)
	{
		...
		else if (value.IsArray())
		{
			...
			{
				values.emplace_back(CreateJsonValueFromRapidJsonValue(doc, *iter));
			}
		...
		else if (value.IsObject())
		{
			...
			{
				values.emplace_back(iter->name.GetString(), CreateJsonValueFromRapidJsonValue(doc, iter->value));
			}
			ret->Value(std::move(values));
			return ret;
		}
