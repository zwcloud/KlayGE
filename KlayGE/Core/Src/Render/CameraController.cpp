...
namespace KlayGE
{
	...
	CameraPathControllerPtr LoadCameraPath(ResIdentifier& res)
	{
		...
		std::unique_ptr<DomDocument> doc = ParseXmlDocument(res);
		DomNode const* root = doc->RootNode();

		if (DomAttrib const* attr = root->Attrib("frame_rate"))
		{
			ret->FrameRate(attr->ValueUInt());
		}

		for (DomNode const* curve_node = root->FirstChildNode("curve"); curve_node; curve_node = curve_node->NextSibling("curve"))
		{
			...
			for (DomNode const* key_node = curve_node->FirstChildNode("key"); key_node; key_node = key_node->NextSibling("key"))
			{
				...
				bool corner;
				if (DomAttrib const* attr = key_node->Attrib("corner"))
				{
	...
	void SaveCameraPath(std::ostream& os, CameraPathControllerPtr const & path)
	{
		...
		for (uint32_t curve_id = 0; curve_id < path->NumCurves(); ++ curve_id)
		{
			...
			{
				auto key_node = doc.AllocNode(DomNodeType::Element, "key");
