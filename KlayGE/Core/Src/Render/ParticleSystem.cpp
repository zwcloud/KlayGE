...
namespace
{
	...
	public:
		...
		void SubThreadStage() override
		{
			...
			std::unique_ptr<DomDocument> doc = ParseXmlDocument(*psmm_input);
			DomNode const* root = doc->RootNode();

			{
				DomNode const* particle_node = root->FirstChildNode("particle");
				{
					DomNode const* alpha_node = particle_node->FirstChildNode("alpha");
				...
				{
					DomNode const* color_node = particle_node->FirstChildNode("color");
					{
						Color from;
						if (DomAttrib const* attr = color_node->Attrib("from"))
						{
						...
						Color to;
						if (DomAttrib const* attr = color_node->Attrib("to"))
						{
							std::string_view const value_str = attr->ValueString();
							std::vector<std::string_view> strs = StringUtil::Split(value_str, StringUtil::EqualTo(' '));
							for (size_t i = 0; i < 3; ++ i)
			...
			{
				DomNode const* emitter_node = root->FirstChildNode("emitter");

				if (DomAttrib const* type_attr = emitter_node->Attrib("type"))
				{
				...
				if (DomNode const* freq_node = emitter_node->FirstChildNode("frequency"))
				{
					DomAttrib const* attr = freq_node->Attrib("value");
					ps_desc_.ps_data->frequency = attr->ValueFloat();
				}
				if (DomNode const* angle_node = emitter_node->FirstChildNode("angle"))
				{
					DomAttrib const* attr = angle_node->Attrib("value");
					ps_desc_.ps_data->angle = attr->ValueInt() * DEG2RAD;
				}
				if (DomNode const* pos_node = emitter_node->FirstChildNode("pos"))
				{
					float3 min_pos(0, 0, 0);
					if (DomAttrib const* attr = pos_node->Attrib("min"))
					{
						std::string_view const value_str = attr->ValueString();
						std::vector<std::string_view> strs = StringUtil::Split(value_str, StringUtil::EqualTo(' '));
						for (size_t i = 0; i < 3; ++ i)
					...
					if (DomAttrib const* attr = pos_node->Attrib("max"))
					{
						std::string_view const value_str = attr->ValueString();
						std::vector<std::string_view> strs = StringUtil::Split(value_str, StringUtil::EqualTo(' '));
						for (size_t i = 0; i < 3; ++ i)
				...
				if (DomNode const* vel_node = emitter_node->FirstChildNode("vel"))
				{
					DomAttrib const* attr = vel_node->Attrib("min");
					ps_desc_.ps_data->min_vel = attr->ValueFloat();

					attr = vel_node->Attrib("max");
					ps_desc_.ps_data->max_vel = attr->ValueFloat();
				}
				if (DomNode const* life_node = emitter_node->FirstChildNode("life"))
				{
					DomAttrib const* attr = life_node->Attrib("min");
			...
			{
				DomNode const* updater_node = root->FirstChildNode("updater");

				if (DomAttrib const* type_attr = updater_node->Attrib("type"))
				{
				...
				{
					for (DomNode const* node = updater_node->FirstChildNode("curve"); node; node = node->NextSibling("curve"))
					{
						std::vector<float2> xys;
						for (DomNode const* ctrl_point_node = node->FirstChildNode("ctrl_point"); ctrl_point_node;
							 ctrl_point_node = ctrl_point_node->NextSibling("ctrl_point"))
						{
							DomAttrib const* attr_x = ctrl_point_node->Attrib("x");
							DomAttrib const* attr_y = ctrl_point_node->Attrib("y");
...
namespace KlayGE
{
	...
	void SaveParticleSystem(ParticleSystemPtr const & ps, std::string const & psml_name)
	{
		...
		auto root = doc.AllocNode(DomNodeType::Element, "particle_system");

		{
			auto particle_node = doc.AllocNode(DomNodeType::Element, "particle");
			{
				auto alpha_node = doc.AllocNode(DomNodeType::Element, "alpha");
			...
			{
				auto color_node = doc.AllocNode(DomNodeType::Element, "color");
				{
		...
		for (uint32_t i = 0; i < ps->NumEmitters(); ++ i)
		{
			...
			{
				auto pos_node = doc.AllocNode(DomNodeType::Element, "pos");
			...
			{
				auto vel_node = doc.AllocNode(DomNodeType::Element, "vel");
			...
			{
				auto life_node = doc.AllocNode(DomNodeType::Element, "life");
		...
		for (uint32_t i = 0; i < ps->NumUpdaters(); ++ i)
		{
			...
			auto updater_node = doc.AllocNode(DomNodeType::Element, "updater");
			...
			if ("polyline" == particle_updater->Type())
			{
				...
				auto size_over_life_node = doc.AllocNode(DomNodeType::Element, "curve");
				...
				for (size_t j = 0; j < size_over_life.size(); ++ j)
				{
					...
					auto ctrl_point_node = doc.AllocNode(DomNodeType::Element, "ctrl_point");
				...
				auto mass_over_life_node = doc.AllocNode(DomNodeType::Element, "curve");
				...
				auto opacity_over_life_node = doc.AllocNode(DomNodeType::Element, "curve");
