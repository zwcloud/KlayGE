...
namespace
{
	...
	public:
		...
		void SubThreadStage() override
		{
			...
			std::unique_ptr<DomDocument> doc = ParseXmlDocument(*mtl_input);
			DomNode const* root = doc->RootNode();

			if (DomAttrib const* attr = root->Attrib("name"))
			{
			...
			if (DomNode const* albedo_node = root->FirstChildNode("albedo"))
			{
			...
			if (DomNode const* metalness_glossiness_node = root->FirstChildNode("metalness_glossiness"))
			{
			...
			{
				if (DomNode const* metalness_node = root->FirstChildNode("metalness"))
				{
				...
				if (DomNode const* glossiness_node = root->FirstChildNode("glossiness"))
				{
			...
			if (DomNode const* emissive_node = root->FirstChildNode("emissive"))
			{
			...
			if (DomNode const* normal_node = root->FirstChildNode("normal"))
			{
				...
				if (DomAttrib const* attr = normal_node->Attrib("scale"))
				{
					mtl_desc_.mtl_data->normal_scale = attr->ValueFloat();
				}
			}
			if (DomNode const* height_node = root->FirstChildNode("height"))
			{
			...
			if (DomNode const* occlusion_node = root->FirstChildNode("occlusion"))
			{
				if (DomAttrib const* attr = occlusion_node->Attrib("texture"))
				{
					mtl_desc_.mtl_data->tex_names[RenderMaterial::TS_Occlusion] = std::string(attr->ValueString());
				}
				if (DomAttrib const* attr = occlusion_node->Attrib("strength"))
				{
					mtl_desc_.mtl_data->occlusion_strength = attr->ValueFloat();
				}
			}
			if (DomNode const* detail_node = root->FirstChildNode("detail"))
			{
			...
			if (DomNode const* transparent_node = root->FirstChildNode("transparent"))
			{
				if (DomAttrib const* attr = transparent_node->Attrib("value"))
				{
					mtl_desc_.mtl_data->transparent = attr->ValueInt() ? true : false;
				}
			}
			if (DomNode const* alpha_test_node = root->FirstChildNode("alpha_test"))
			{
				if (DomAttrib const* attr = alpha_test_node->Attrib("value"))
				{
					mtl_desc_.mtl_data->alpha_test = attr->ValueFloat();
				}
			}
			if (DomNode const* sss_node = root->FirstChildNode("sss"))
			{
				if (DomAttrib const* attr = sss_node->Attrib("value"))
				{
					mtl_desc_.mtl_data->sss = attr->ValueInt() ? true : false;
				}
			}
			if (DomNode const* two_sided_node = root->FirstChildNode("two_sided"))
			{
...
namespace KlayGE
{
	...
	void SaveRenderMaterial(RenderMaterialPtr const & mtl, std::string const & mtlml_name)
	{
		...
		auto root = doc.AllocNode(DomNodeType::Element, "material");

		{
			auto albedo_node = doc.AllocNode(DomNodeType::Element, "albedo");
		...
		{
			auto metalness_glossiness_node = doc.AllocNode(DomNodeType::Element, "metalness_glossiness");
		...
		{
			auto emissive_node = doc.AllocNode(DomNodeType::Element, "emissive");
		...
		{
			auto normal_node = doc.AllocNode(DomNodeType::Element, "normal");
		...
		{
			auto height_node = doc.AllocNode(DomNodeType::Element, "height");
		...
		{
			auto occlusion_node = doc.AllocNode(DomNodeType::Element, "occlusion");
		...
		{
			auto detail_node = doc.AllocNode(DomNodeType::Element, "detail");
			...
			{
				auto tess_node = doc.AllocNode(DomNodeType::Element, "tess");
		...
		if (mtl->Transparent())
		{
			auto transparent_node = doc.AllocNode(DomNodeType::Element, "transparent");
		...
		{
			auto alpha_test_node = doc.AllocNode(DomNodeType::Element, "alpha_test");
		...
		if (mtl->Sss())
		{
			auto sss_node = doc.AllocNode(DomNodeType::Element, "sss");
		...
		if (mtl->TwoSided())
		{
			auto two_sided_node = doc.AllocNode(DomNodeType::Element, "two_sided");
