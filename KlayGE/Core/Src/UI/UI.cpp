...
namespace
{
	...
	bool ReadBool(KlayGE::DomNode const& node, std::string const & name, bool default_val)
	{
		bool ret = default_val;

		if (KlayGE::DomAttrib const* attr = node.Attrib(name))
		{
...
namespace KlayGE
{
	...
	void UIManager::Load(ResIdentifier& source)
	{
		...
		for (DomNode const* node = root->FirstChildNode("include"); node;)
		{
			...
			include_docs.push_back(ParseXmlDocument(*ResLoader::Instance().Open(std::string(attr->ValueString()))));
			...
			{
				if (DomNodeType::Element == child_node->Type())
				{
		...
		for (DomNode const* node = root->FirstChildNode("dialog"); node; node = node->NextSibling("dialog"))
		{
			UIDialogPtr dlg;
			{
				int32_t x, y;
				...
				if (DomAttrib const* attr = node->Attrib("align_x"))
				{
				...
				if (DomAttrib const* attr = node->Attrib("align_y"))
				{
				...
				if (DomAttrib const* attr = node->Attrib("bg_color_r"))
				{
					bg_clr.r() = attr->ValueFloat();
				}
				if (DomAttrib const* attr = node->Attrib("bg_color_g"))
				{
					bg_clr.g() = attr->ValueFloat();
				}
				if (DomAttrib const* attr = node->Attrib("bg_color_b"))
				{
					bg_clr.b() = attr->ValueFloat();
				}
				if (DomAttrib const* attr = node->Attrib("bg_color_a"))
				{
			...
			for (DomNode const* ctrl_node = node->FirstChildNode("control"); ctrl_node; ctrl_node = ctrl_node->NextSibling("control"))
			{
			...
			for (DomNode const* ctrl_node = node->FirstChildNode("control"); ctrl_node; ctrl_node = ctrl_node->NextSibling("control"))
			{
				...
				if (DomAttrib const* attr = ctrl_node->Attrib("align_x"))
				{
					std::string_view const align_x_str = attr->ValueString();
					if ("left" == align_x_str)
					{
				...
				if (DomAttrib const* attr = ctrl_node->Attrib("align_y"))
				{
					std::string_view const align_y_str = attr->ValueString();
					if ("top" == align_y_str)
					{
				...
				{
					TexturePtr tex;
					if (DomAttrib const* attr = ctrl_node->Attrib("texture"))
					{
				...
				else if (CT_HASH("list_box") == type_str_hash)
				{
					...
					if (DomAttrib const* attr = ctrl_node->Attrib("style"))
					{
					...
					for (DomNode const* item_node = ctrl_node->FirstChildNode("item"); item_node; item_node = item_node->NextSibling("item"))
					{
					...
					if (DomAttrib const* attr = ctrl_node->Attrib("selected"))
					{
