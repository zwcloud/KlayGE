...
namespace
{
	...
	private:
		...
		// From MeshML
		void CompileMaterialsChunk(DomNode const& materials_chunk);
		void CompileMeshBoundingBox(DomNode const& mesh_node, uint32_t mesh_index, bool& recompute_pos_bb, bool& recompute_tc_bb);
		void CompileMeshesChunk(DomNode const& meshes_chunk);
		void CompileMeshLodChunk(DomNode const& lod_node, uint32_t mesh_index, uint32_t lod, bool recompute_pos_bb, bool recompute_tc_bb);
		void CompileMeshesVerticesChunk(
			DomNode const& vertices_chunk, uint32_t mesh_index, uint32_t lod, bool recompute_pos_bb, bool recompute_tc_bb);
		void CompileMeshesTrianglesChunk(DomNode const& triangles_chunk, uint32_t mesh_index, uint32_t lod);
		void CompileBonesChunk(DomNode const& bones_chunk);
		void CompileKeyFramesChunk(DomNode const& key_frames_chunk);
		void CompileBBKeyFramesChunk(DomNode const* bb_kfs_chunk, uint32_t mesh_index);
		void CompileActionsChunk(DomNode const& animations_chunk);
	...
	void MeshLoader::CompileMaterialsChunk(DomNode const& materials_chunk)
	{
		uint32_t num_mtls = 0;
		for (DomNode const* mtl_node = materials_chunk.FirstChildNode("material"); mtl_node; mtl_node = mtl_node->NextSibling("material"))
		{
			++ num_mtls;
		}
		...
		for (DomNode const* mtl_node = materials_chunk.FirstChildNode("material"); mtl_node;
			 mtl_node = mtl_node->NextSibling("material"), ++mtl_index)
		{
			...
			if (DomAttrib const* attr = mtl_node->Attrib("name"))
			{
				mtl.Name(attr->ValueString());
			}

			if (DomNode const* albedo_node = mtl_node->FirstChildNode("albedo"))
			{
				if (DomAttrib const* attr = albedo_node->Attrib("color"))
				{
					float4 albedo;
					ExtractFVector<4>(attr->ValueString(), &albedo[0]);
					mtl.Albedo(albedo);
				}
				if (DomAttrib const* attr = albedo_node->Attrib("texture"))
				{
			...
			else
			{
				...
				if (DomAttrib const* diffuse_attr = mtl_node->Attrib("diffuse"))
				{
					ExtractFVector<3>(diffuse_attr->ValueString(), &albedo[0]);
				}
				else
				{
					if (DomAttrib const* attr = mtl_node->Attrib("diffuse_r"))
					{
						albedo.x() = attr->ValueFloat();
					}
					if (DomAttrib const* attr = mtl_node->Attrib("diffuse_g"))
					{
						albedo.y() = attr->ValueFloat();
					}
					if (DomAttrib const* attr = mtl_node->Attrib("diffuse_b"))
					{
						albedo.z() = attr->ValueFloat();
					}
				}

				if (DomAttrib const* attr = mtl_node->Attrib("opacity"))
				{
			...
			if (DomNode const* metalness_glossiness_node = mtl_node->FirstChildNode("metalness_glossiness"))
			{
				if (DomAttrib const* attr = metalness_glossiness_node->Attrib("metalness"))
				{
					mtl.Metalness(attr->ValueFloat());
				}
				if (DomAttrib const* attr = metalness_glossiness_node->Attrib("glossiness"))
				{
					mtl.Glossiness(attr->ValueFloat());
				}
				if (DomAttrib const* attr = metalness_glossiness_node->Attrib("texture"))
				{
			...
			{
				if (DomNode const* metalness_node = mtl_node->FirstChildNode("metalness"))
				{
					if (DomAttrib const* attr = metalness_node->Attrib("value"))
					{
						mtl.Metalness(attr->ValueFloat());
					}
					if (DomAttrib const* attr = metalness_node->Attrib("texture"))
					{
						mtl.TextureName(RenderMaterial::TS_MetalnessGlossiness, std::string(attr->ValueString()));
					}
				}

				if (DomNode const* glossiness_node = mtl_node->FirstChildNode("glossiness"))
				{
					if (DomAttrib const* attr = glossiness_node->Attrib("value"))
					{
						mtl.Glossiness(attr->ValueFloat());
					}
					if (DomAttrib const* attr = glossiness_node->Attrib("texture"))
					{
						mtl.TextureName(RenderMaterial::TS_MetalnessGlossiness, std::string(attr->ValueString()));
					}
				}
				...
				{
					if (DomAttrib const* attr = mtl_node->Attrib("shininess"))
					{
			...
			if (DomNode const* emissive_node = mtl_node->FirstChildNode("emissive"))
			{
				if (DomAttrib const* attr = emissive_node->Attrib("color"))
				{
					float3 emissive;
					ExtractFVector<3>(attr->ValueString(), &emissive[0]);
					mtl.Emissive(emissive);
				}
				if (DomAttrib const* attr = emissive_node->Attrib("texture"))
				{
			...
			{
				float3 emissive(0, 0, 0);
				if (DomAttrib const* emit_attr = mtl_node->Attrib("emit"))
				{
					ExtractFVector<3>(emit_attr->ValueString(), &emissive[0]);
				}
				else
				{
					if (DomAttrib const* attr = mtl_node->Attrib("emit_r"))
					{
						emissive.x() = attr->ValueFloat();
					}
					if (DomAttrib const* attr = mtl_node->Attrib("emit_g"))
					{
						emissive.y() = attr->ValueFloat();
					}
					if (DomAttrib const* attr = mtl_node->Attrib("emit_b"))
					{
			...
			if (DomNode const* normal_node = mtl_node->FirstChildNode("normal"))
			{
				if (DomAttrib const* attr = normal_node->Attrib("texture"))
				{
					mtl.TextureName(RenderMaterial::TS_Normal, std::string(attr->ValueString()));
				}
			}

			DomNode const* height_node = mtl_node->FirstChildNode("height");
			if (!height_node)
			{
				height_node = mtl_node->FirstChildNode("bump");
			}
			if (height_node)
			{
				if (DomAttrib const* attr = height_node->Attrib("texture"))
				{
					mtl.TextureName(RenderMaterial::TS_Height, std::string(attr->ValueString()));
				}
				if (DomAttrib const* attr = height_node->Attrib("offset"))
				{
					mtl.HeightOffset(attr->ValueFloat());
				}
				if (DomAttrib const* attr = height_node->Attrib("scale"))
				{
					mtl.HeightScale(attr->ValueFloat());
				}
			}

			if (DomNode const* detail_node = mtl_node->FirstChildNode("detail"))
			{
				if (DomAttrib const* attr = detail_node->Attrib("mode"))
				{
				...
				if (DomAttrib const* attr = detail_node->Attrib("height_offset"))
				{
					mtl.HeightOffset(attr->ValueFloat());
				}
				if (DomAttrib const* attr = detail_node->Attrib("height_scale"))
				{
					mtl.HeightScale(attr->ValueFloat());
				}

				if (DomNode const* tess_node = detail_node->FirstChildNode("tess"))
				{
					if (DomAttrib const* attr = tess_node->Attrib("edge_hint"))
					{
						mtl.EdgeTessHint(attr->ValueFloat());
					}
					if (DomAttrib const* attr = tess_node->Attrib("inside_hint"))
					{
						mtl.InsideTessHint(attr->ValueFloat());
					}
					if (DomAttrib const* attr = tess_node->Attrib("min"))
					{
						mtl.MinTessFactor(attr->ValueFloat());
					}
					if (DomAttrib const* attr = tess_node->Attrib("max"))
					{
				...
				{
					if (DomAttrib const* attr = detail_node->Attrib("edge_tess_hint"))
					{
						mtl.EdgeTessHint(attr->ValueFloat());
					}
					if (DomAttrib const* attr = detail_node->Attrib("inside_tess_hint"))
					{
						mtl.InsideTessHint(attr->ValueFloat());
					}
					if (DomAttrib const* attr = detail_node->Attrib("min_tess"))
					{
						mtl.MinTessFactor(attr->ValueFloat());
					}
					if (DomAttrib const* attr = detail_node->Attrib("max_tess"))
					{
						mtl.MaxTessFactor(attr->ValueFloat());
					}
				}
			...
			if (DomNode const* transparent_node = mtl_node->FirstChildNode("transparent"))
			{
				DomAttrib const* attr = transparent_node->Attrib("value");
				if (attr)
				{
			...
			if (DomNode const* alpha_test_node = mtl_node->FirstChildNode("alpha_test"))
			{
				DomAttrib const* attr = alpha_test_node->Attrib("value");
				if (attr)
				{
			...
			if (DomNode const* sss_node = mtl_node->FirstChildNode("sss"))
			{
				DomAttrib const* attr = sss_node->Attrib("value");
				if (attr)
				{
			...
			{
				DomAttrib const* attr = mtl_node->Attrib("sss");
				if (attr)
				{
					mtl.Sss(attr->ValueInt() ? true : false);
				}
			...
			if (DomNode const* two_sided_node = mtl_node->FirstChildNode("two_sided"))
			{
				if (DomAttrib const* attr = two_sided_node->Attrib("value"))
				{
					mtl.TwoSided(attr->ValueInt() ? true : false);
				}
			}

			DomNode const* tex_node = mtl_node->FirstChildNode("texture");
			if (!tex_node)
			{
				if (DomNode const* textures_chunk = mtl_node->FirstChildNode("textures_chunk"))
				{
					tex_node = textures_chunk->FirstChildNode("texture");
				}
			}
			if (tex_node)
			{
	...
	void MeshLoader::CompileMeshBoundingBox(DomNode const& mesh_node, uint32_t mesh_index, bool& recompute_pos_bb, bool& recompute_tc_bb)
	{
		if (DomNode const* pos_bb_node = mesh_node.FirstChildNode("pos_bb"))
		{
			float3 pos_min_bb, pos_max_bb;
			if (DomAttrib const* attr = pos_bb_node->Attrib("min"))
			{
				ExtractFVector<3>(attr->ValueString(), &pos_min_bb[0]);
			}
			else
			{
				DomNode const* pos_min_node = pos_bb_node->FirstChildNode("min");
			...
			if (DomAttrib const* attr = pos_bb_node->Attrib("max"))
			{
				ExtractFVector<3>(attr->ValueString(), &pos_max_bb[0]);
			}
			else
			{
				DomNode const* pos_max_node = pos_bb_node->FirstChildNode("max");
		...
		if (DomNode const* tc_bb_node = mesh_node.FirstChildNode("tc_bb"))
		{
			float3 tc_min_bb, tc_max_bb;
			if (DomAttrib const* attr = tc_bb_node->Attrib("min"))
			{
				ExtractFVector<2>(attr->ValueString(), &tc_min_bb[0]);
			}
			else
			{
				DomNode const* tc_min_node = tc_bb_node->FirstChildNode("min");
				tc_min_bb.x() = tc_min_node->Attrib("x")->ValueFloat();
				tc_min_bb.y() = tc_min_node->Attrib("y")->ValueFloat();
			}
			if (DomAttrib const* attr = tc_bb_node->Attrib("max"))
			{
				ExtractFVector<2>(attr->ValueString(), &tc_max_bb[0]);
			}
			else
			{
				DomNode const* tc_max_node = tc_bb_node->FirstChildNode("max");
	...
	void MeshLoader::CompileMeshesChunk(DomNode const& meshes_chunk)
	{
		uint32_t num_meshes = 0;
		for (DomNode const* mesh_node = meshes_chunk.FirstChildNode("mesh"); mesh_node; mesh_node = mesh_node->NextSibling("mesh"))
		{
			++ num_meshes;
		}
		...
		uint32_t mesh_index = 0;
		for (DomNode const* mesh_node = meshes_chunk.FirstChildNode("mesh"); mesh_node; mesh_node = mesh_node->NextSibling("mesh"), ++ mesh_index)
		{
			...
			{
				if (DomNode const* vertices_chunk = mesh_node->FirstChildNode("vertices_chunk"))
				{
					this->CompileMeshBoundingBox(*vertices_chunk, mesh_index, recompute_pos_bb, recompute_tc_bb);
				}
			}

			if (DomNode const* lod_node = mesh_node->FirstChildNode("lod"))
			{
				uint32_t mesh_lod = 0;
				...
				std::vector<DomNode const*> lod_nodes(mesh_lod);
				for (lod_node = mesh_node->FirstChildNode("lod"); lod_node; lod_node = lod_node->NextSibling("lod"))
				{
	...
	void MeshLoader::CompileMeshLodChunk(
		DomNode const& lod_node, uint32_t mesh_index, uint32_t lod, bool recompute_pos_bb, bool recompute_tc_bb)
	{
		if (DomNode const* vertices_chunk = lod_node.FirstChildNode("vertices_chunk"))
		{
			this->CompileMeshesVerticesChunk(*vertices_chunk, mesh_index, lod, recompute_pos_bb, recompute_tc_bb);
		}

		if (DomNode const* triangles_chunk = lod_node.FirstChildNode("triangles_chunk"))
		{
		...
		DomNode const& vertices_chunk, uint32_t mesh_index, uint32_t lod, bool recompute_pos_bb, bool recompute_tc_bb)
	{
		...
		for (DomNode const* vertex_node = vertices_chunk.FirstChildNode("vertex"); vertex_node; vertex_node = vertex_node->NextSibling("vertex"))
		{
			{
				float3 pos;
				if (DomAttrib const* x_attr = vertex_node->Attrib("x"))
				{
					pos.x() = x_attr->ValueFloat();
					pos.y() = vertex_node->Attrib("y")->ValueFloat();
					pos.z() = vertex_node->Attrib("z")->ValueFloat();

					if (DomAttrib const* u_attr = vertex_node->Attrib("u"))
					{
						float3 tex_coord;
			...
			if (DomNode const* diffuse_node = vertex_node->FirstChildNode("diffuse"))
			{
				has_diffuse = true;

				float4 diffuse;
				if (DomAttrib const* attr = diffuse_node->Attrib("v"))
				{
			...
			if (DomNode const* specular_node = vertex_node->FirstChildNode("specular"))
			{
				has_specular = true;

				float3 specular;
				if (DomAttrib const* attr = specular_node->Attrib("v"))
				{
			...
			{
				if (DomNode const* tex_coord_node = vertex_node->FirstChildNode("tex_coord"))
				{
					has_tex_coord = true;

					float3 tex_coord;
					if (DomAttrib const* attr = tex_coord_node->Attrib("u"))
					{
			...
			if (DomNode const* weight_node = vertex_node->FirstChildNode("weight"))
			{
				std::vector<std::pair<uint32_t, float>> binding;

				DomAttrib const* attr = weight_node->Attrib("joint");
				if (!attr)
				{
				...
				if (attr)
				{
					DomAttrib const* weight_attr = weight_node->Attrib("weight");
			...
			if (DomNode const* normal_node = vertex_node->FirstChildNode("normal"))
			{
				has_normal = true;

				float3 normal;
				if (DomAttrib const* attr = normal_node->Attrib("v"))
				{
			...
			if (DomNode const* tangent_node = vertex_node->FirstChildNode("tangent"))
			{
				has_tangent = true;

				float4 tangent;
				if (DomAttrib const* attr = tangent_node->Attrib("v"))
				{
			...
			if (DomNode const* binormal_node = vertex_node->FirstChildNode("binormal"))
			{
				has_binormal = true;

				float3 binormal;
				if (DomAttrib const* attr = binormal_node->Attrib("v"))
				{
			...
			if (DomNode const* tangent_quat_node = vertex_node->FirstChildNode("tangent_quat"))
			{
				has_tangent_quat = true;

				Quaternion tangent_quat;
				if (DomAttrib const* attr = tangent_quat_node->Attrib("v"))
				{
	...
	void MeshLoader::CompileMeshesTrianglesChunk(DomNode const& triangles_chunk, uint32_t mesh_index, uint32_t lod)
	{
		auto& mesh = meshes_[mesh_index];
		auto& mesh_lod = mesh.lods[lod];

		for (DomNode const* tri_node = triangles_chunk.FirstChildNode("triangle"); tri_node; tri_node = tri_node->NextSibling("triangle"))
		{
			uint32_t ind[3];
			if (DomAttrib const* attr = tri_node->Attrib("index"))
			{
	...
	void MeshLoader::CompileBonesChunk(DomNode const& bones_chunk)
	{
		for (DomNode const* bone_node = bones_chunk.FirstChildNode("bone"); bone_node; bone_node = bone_node->NextSibling("bone"))
		{
			auto joint = MakeSharedPtr<JointComponent>();
			...
			if (DomNode const* bind_pos_node = bone_node->FirstChildNode("bind_pos"))
			{
				float3 bind_pos(bind_pos_node->Attrib("x")->ValueFloat(), bind_pos_node->Attrib("y")->ValueFloat(),
					bind_pos_node->Attrib("z")->ValueFloat());

				DomNode const* bind_quat_node = bone_node->FirstChildNode("bind_quat");
			...
			{
				DomNode const* bind_real_node = bone_node->FirstChildNode("real");
				if (!bind_real_node)
				{
					bind_real_node = bone_node->FirstChildNode("bind_real");
				}
				if (DomAttrib const* attr = bind_real_node->Attrib("v"))
				{
				...
				DomNode const* bind_dual_node = bone_node->FirstChildNode("dual");
				if (!bind_dual_node)
				{
					bind_dual_node = bone_node->FirstChildNode("bind_dual");
				}
				if (DomAttrib const* attr = bind_dual_node->Attrib("v"))
				{
	...
	void MeshLoader::CompileKeyFramesChunk(DomNode const& key_frames_chunk)
	{
		auto& skinned_model = checked_cast<SkinnedModel&>(*render_model_);

		if (DomAttrib const* nf_attr = key_frames_chunk.Attrib("num_frames"))
		{
		...
		uint32_t joint_id = 0;
		for (DomNode const* kf_node = key_frames_chunk.FirstChildNode("key_frame"); kf_node; kf_node = kf_node->NextSibling("key_frame"))
		{
			if (DomAttrib const* joint_attr = kf_node->Attrib("joint"))
			{
			...
			int32_t frame_id = -1;
			for (DomNode const* key_node = kf_node->FirstChildNode("key"); key_node; key_node = key_node->NextSibling("key"))
			{
				if (DomAttrib const* id_attr = key_node->Attrib("id"))
				{
				...
				Quaternion bind_real, bind_dual;
				float bind_scale;
				if (DomNode const* pos_node = key_node->FirstChildNode("pos"))
				{
					float3 bind_pos(pos_node->Attrib("x")->ValueFloat(), pos_node->Attrib("y")->ValueFloat(),
						pos_node->Attrib("z")->ValueFloat());

					DomNode const* quat_node = key_node->FirstChildNode("quat");
				...
				{
					DomNode const* bind_real_node = key_node->FirstChildNode("real");
					if (!bind_real_node)
					{
						bind_real_node = key_node->FirstChildNode("bind_real");
					}
					if (DomAttrib const* attr = bind_real_node->Attrib("v"))
					{
					...
					DomNode const* bind_dual_node = key_node->FirstChildNode("dual");
					if (!bind_dual_node)
					{
						bind_dual_node = key_node->FirstChildNode("bind_dual");
					}
					if (DomAttrib const* attr = bind_dual_node->Attrib("v"))
					{
	...
	void MeshLoader::CompileBBKeyFramesChunk(DomNode const* bb_kfs_chunk, uint32_t mesh_index)
	{
		auto& skinned_model = checked_cast<SkinnedModel&>(*render_model_);
		...
		if (bb_kfs_chunk)
		{
			for (DomNode const* bb_kf_node = bb_kfs_chunk->FirstChildNode("bb_key_frame"); bb_kf_node;
				bb_kf_node = bb_kf_node->NextSibling("bb_key_frame"))
			{
				...
				int32_t frame_id = -1;
				for (DomNode const* key_node = bb_kf_node->FirstChildNode("key"); key_node; key_node = key_node->NextSibling("key"))
				{
					if (DomAttrib const* id_attr = key_node->Attrib("id"))
					{
						frame_id = id_attr->ValueInt();
	...
	void MeshLoader::CompileActionsChunk(DomNode const& actions_chunk)
	{
		auto& skinned_model = checked_cast<SkinnedModel&>(*render_model_);
		...
		Animation animation;
		if (DomNode const* action_node = actions_chunk.FirstChildNode("action"))
		{
	...
	void MeshLoader::LoadFromMeshML(std::string_view input_name, MeshMetadata const & metadata)
	{
		...
		std::unique_ptr<DomDocument> doc = ParseXmlDocument(*file);
		DomNode const* root = doc->RootNode();

		BOOST_ASSERT(root->Attrib("version") && (root->Attrib("version")->ValueInt() >= 1));

		if (DomNode const* bones_chunk = root->FirstChildNode("bones_chunk"))
		{
		...
		if (DomNode const* meshes_chunk = root->FirstChildNode("meshes_chunk"))
		{
		...
		if (DomNode const* materials_chunk = root->FirstChildNode("materials_chunk"))
		{
			this->CompileMaterialsChunk(*materials_chunk);
		}

		if (DomNode const* key_frames_chunk = root->FirstChildNode("key_frames_chunk"))
		{
			...
			DomNode const* bb_kfs_chunk = root->FirstChildNode("bb_key_frames_chunk");
		...
		if (DomNode const* actions_chunk = root->FirstChildNode("actions_chunk"))
		{
