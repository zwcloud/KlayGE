...
namespace
{
	...
	int RetrieveAttrValue(DomNode const& node, std::string const & attr_name, int default_value)
	{
		if (DomAttrib const* attr = node.Attrib(attr_name))
		{
	...
	std::string RetrieveAttrValue(DomNode const& node, std::string const& attr_name, std::string const& default_value)
	{
		if (DomAttrib const* attr = node.Attrib(attr_name))
		{
	...
	int RetrieveNodeValue(DomNode const& root, std::string const& node_name, int default_value)
	{
		if (DomNode const* node = root.FirstChildNode(node_name))
		{
	...
	std::string RetrieveNodeValue(DomNode const& root, std::string const& node_name, std::string const& default_value)
	{
		if (DomNode const* node = root.FirstChildNode(node_name))
		{
			return RetrieveAttrValue(*node, "value", default_value);
...
namespace KlayGE
{
	...
	PlatformDefinition::PlatformDefinition(std::string_view name)
	{
		...
		std::unique_ptr<DomDocument> doc = ParseXmlDocument(*plat);
		...
		DomNode const* max_shader_model_node = root->FirstChildNode("max_shader_model");
