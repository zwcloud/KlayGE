...
void ScenePlayerApp::LoadScene(std::string const & name)
{
	...
	std::unique_ptr<DomDocument> doc = ParseXmlDocument(*ifs);
	DomNode const* root = doc->RootNode();

	if (DomAttrib const* attr = root->Attrib("skybox"))
	{
	...
	for (DomNode const* light_node = root->FirstChildNode("light"); light_node; light_node = light_node->NextSibling("light"))
	{
		...
		if (DomNode const* attr_node = light_node->FirstChildNode("attr"))
		{
		...
		if (DomNode const* color_node = light_node->FirstChildNode("color"))
		{
			float3 color;
		...
		{
			if (DomNode const* dir_node = light_node->FirstChildNode("dir"))
			{
		...
		{
			if (DomNode const* pos_node = light_node->FirstChildNode("pos"))
			{
			...
			if (DomNode const* fall_off_node = light_node->FirstChildNode("fall_off"))
			{
				float3 fall_off;
			...
			if ((LightSource::LT_Point == light->Type()) || (LightSource::LT_Spot == light->Type()))
			{
				if (DomNode const* projective_node = light_node->FirstChildNode("projective"))
				{
					if (DomAttrib const* attr = projective_node->Attrib("name"))
					{
				...
				{
					if (DomNode const* angle_node = light_node->FirstChildNode("angle"))
					{
		...
		if (DomNode const* update_node = light_node->FirstChildNode("update"))
		{
			update_node = update_node->FirstChildNode();
			if (update_node && (DomNodeType::CData == update_node->Type()))
			{
		...
		if (DomNode const* scale_node = light_node->FirstChildNode("scale"))
		{
			float3 scale(1, 1, 1);
			{
	...
	for (DomNode const* model_node = root->FirstChildNode("model"); model_node; model_node = model_node->NextSibling("model"))
	{
		...
		if (DomNode const* scale_node = model_node->FirstChildNode("scale"))
		{
			auto v = scale_node->Attrib("v")->ValueString();
			MemInputStreamBuf stream_buff(v.data(), v.size());
			std::istream(&stream_buff) >> scale.x() >> scale.y() >> scale.z();
		...
		if (DomNode const* rotate_node = model_node->FirstChildNode("rotate"))
		{
		...
		if (DomNode const* translate_node = model_node->FirstChildNode("translate"))
		{
		...
		if (DomNode const* attribute_node = model_node->FirstChildNode("attr"))
		{
			if (DomAttrib const* attr = attribute_node->Attrib("value"))
			{
		...
		if (DomNode const* update_node = model_node->FirstChildNode("update"))
		{
			update_node = update_node->FirstChildNode();
			if (update_node && (DomNodeType::CData == update_node->Type()))
			{
		...
		DomAttrib const* attr = model_node->Attrib("model");
		BOOST_ASSERT(attr);
...

	{
		...
		DomNode const* camera_node = root->FirstChildNode("camera");

		if (DomNode const* eye_pos_node = camera_node->FirstChildNode("eye_pos"))
		{
			auto v = eye_pos_node->Attrib("v")->ValueString();
			MemInputStreamBuf stream_buff(v.data(), v.size());
			std::istream(&stream_buff) >> eye_pos.x() >> eye_pos.y() >> eye_pos.z();
		}
		if (DomNode const* look_at_node = camera_node->FirstChildNode("look_at"))
		{
			auto v = look_at_node->Attrib("v")->ValueString();
			MemInputStreamBuf stream_buff(v.data(), v.size());
			std::istream(&stream_buff) >> look_at.x() >> look_at.y() >> look_at.z();
		}
		if (DomNode const* up_node = camera_node->FirstChildNode("up"))
		{
		...
		if (DomNode const* fov_node = camera_node->FirstChildNode("fov"))
		{
			fov = fov_node->Attrib("s")->ValueFloat();
		}
		if (DomNode const* aspect_node = camera_node->FirstChildNode("aspect"))
		{
			aspect = aspect_node->Attrib("s")->ValueFloat();
		}
		if (DomNode const* near_plane_node = camera_node->FirstChildNode("near"))
		{
			near_plane = near_plane_node->Attrib("s")->ValueFloat();
		}
		if (DomNode const* far_plane_node = camera_node->FirstChildNode("far"))
		{
		...
		if (DomNode const* update_node = camera_node->FirstChildNode("update"))
		{
			update_node = update_node->FirstChildNode();
			if (update_node && (DomNodeType::CData == update_node->Type()))
			{
